...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const ease = 0.12;
      setCurrent((prev) => {
        const next = prev + (target.current - prev) * ease;
        return next;
      });
        ...
        <div className="absolute left-0 z-20 h-[50vh] w-full md:hidden px-6 pb-6 flex items-end top-[1px]">
          <section className="w-full max-w-4xl font-poppins text-black flex flex-col items-center space-y-4 text-center">
            ...
            <div className="flex gap-4 flex-wrap justify-center">
              <a
                href="#kontakt"
                className="inline-block bg-black text-white font-semibold px-4 py-2 text-sm sm:px-6 sm:py-3 sm:text-base rounded-full shadow-md hover:scale-105 hover:shadow-lg transition-all duration-300"
              >
                Võta Ühendust
              </a>
              <a
                href="#video"
                className="group relative inline-flex items-center gap-2 border border-black text-black font-semibold px-4 py-2 text-sm sm:px-6 sm:py-3 sm:text-base rounded-full shadow-md hover:bg-black hover:text-white hover:shadow-lg hover:scale-105 transition-all duration-300 overflow-hidden"
              >
                Vaata Videot
